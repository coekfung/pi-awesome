...
- `usage/usage.ts` — shows the active model's provider usage/quota in the status line; adapters resolve auth, fetch, and normalize per-provider data; currently supports OpenAI Codex (e.g. `📊 codex: 59% 5h 61% wk`) and DeepSeek (e.g. `📊 deepseek: ¥59.00 rem`)
